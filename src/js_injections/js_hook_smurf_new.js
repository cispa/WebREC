    ...
    for (let ele of fake.getElementsByTagName("*")) {
        for (let attr of ele.getAttributeNames()) {
            if (attr.startsWith("on")) {
                ...
                if (ON_HANDLERS.indexOf(attr) !== -1) {
                    report("CODE 1 true", ele.outerHTML);
                    report("inline_handler", ele[attr] === null ? ele.getAttribute(attr) : ele[attr].toString());
                } else {
                    report("CODE 1 false", ele.outerHTML);
                    report("on_attr_handler", ele[attr] === null ? ele.getAttribute(attr) : ele[attr].toString());
                }
...
function _hookCall_taint(object, property) {
    try {
        let old_fun = object[property];

        function _hooked() {
            // report(tag, {arguments: Array(...arguments)});
            ourLog('Hooked call for taint: ' + arguments);
...
function _hookArbitratyNodeInsertions(object, property, tag) {
    try {
        ...
        function _hookedNodeInsertion() {
            try {
                for (let relevant_argument of arguments) {
                    if (relevant_argument.parentNode === null) {
                        analyzeElement(relevant_argument, '');
                        for (let element of relevant_argument.getElementsByTagName("*")) {
                            analyzeElement(element, '_nested 2 ' + tag);
                        }
                    }
                }
            } catch (e) {
...
function analyzeElement(element, suffix) {
    try {
        if (element.tagName && element.tagName.toLowerCase() === 'script') {
            if (!element.src && element.text) {
                report('inline_script', {source: element.text});
                reportScript(element.text);
            }
        }
        let programmatic_handlers = new Set();
        for (let attr of ON_HANDLERS) {
            if (element[attr] !== null && element[attr] !== undefined) {
                let sc = element[attr].toString();
                if (!sc.startsWith(`function ${attr}(event)`)) {
                    report("CODE 3a", element.outerHTML);
                    report(`prog_handler${suffix}`, sc);
                    // ourLog(`[BEN] programatic handler detected ${attr} on ${element.outerHTML}: ${element[attr]}`)
                    programmatic_handlers.add(attr);
                }
            }
        }
        ...
        for (let attr of element.getAttributeNames()) {
            if (ON_HANDLERS.indexOf(attr) !== -1) {
                if (programmatic_handlers.has(attr))
                    continue;
                let sc = element.getAttribute(attr).toString();
                // let sc = element[attr] === null ? element.getAttribute(attr) : element[attr].toString();
                // check for nodes

                report("CODE 3b", element.outerHTML);
                report(`inline_handler${suffix}`, sc);
                // ourLog(`[BEN] inline_handler found ${sc}`);
                reportScript(sc);
            }
        }
        ...
        function _hookedNodeInsertion() {
            try {
                if (arguments.length > nodeIndex) {
                    let relevant_argument = arguments[nodeIndex];
                    if (relevant_argument.parentNode === null) {
                        analyzeElement(relevant_argument, '');
                        for (let element of relevant_argument.getElementsByTagName("*")) {
                            analyzeElement(element, '_nested 1 ' + tag);
                        }
                    }
                }
            } catch (e) {
...
// this was only commented out for the test
