...
class DOMElementNode(Node, ABC):
    ...
    def parent_the_first_by_florian(self) -> Optional[ParentDOMNode]:
        incoming_edges = self.incoming_edges()
        for edge in incoming_edges:
            insert_edge = edge.as_insert_edge()
            if not insert_edge:
                continue
    ...
    def domroot_custom_by_florian(self) -> Optional["DOMRootNode"]:        
        current_node = self.parent_the_first_by_florian()
        while current_node:
            if domroot_node := current_node.as_domroot_node():
                return domroot_node
            current_node = current_node.parent_the_first_by_florian()
        return None
