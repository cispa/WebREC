...
from pagegraph.types import ChildDOMNode, ParentDOMNode, FrameId, RequestId, EventListenerId
    ...
    def validate(self) -> bool:
        ...
        if valid_incoming_node_types is not None:
            ...
            if node_type not in valid_incoming_node_types:
                self.throw(f"Unexpected incoming node type for {self.edge_type()} edge: {node_type}")
                return False
        ...
        if valid_outgoing_node_types is not None:
            ...
            if node_type not in valid_outgoing_node_types:
                self.throw(f"Unexpected outgoing node type for {self.edge_type()} edge: {node_type}")
                return False
        return True
...
class AttributeSetEdge(AttributeEdge):
    ...
    def event_listener_id(self) -> EventListenerId | None:
        if Edge.RawAttrs.EVENT_LISTENER_ID.value in self.data():
            return int(self.data()[Edge.RawAttrs.EVENT_LISTENER_ID.value])
        return None
...
class NodeCreateEdge(FrameIdAttributedEdge):
    ...
    def validate(self) -> bool:
        ...
        # not supported for the researched graph version
        return True
